...
# 6 Add version info to the end of all scrip files
version_info=$(extract_version_from_file)
echo $version_info
echo "$version_info" >>"bin/efc_lib"
echo "$version_info" >>"bin/efc_$version.min"
echo "$version_info" >>"bin/efc_$version"
echo "$version_info" >>"bin/efc"

# 7 compress the bin folder into release folder
